...
SQL;
    ...
    public function insertBook($pdo, $book) {
        ...
        $price = $book->getPrice();
        $query = <<<SQL
INSERT INTO bookstore.book (isbn,title,author,stock,price) VALUES ("$isbn","$title","$author","$stock","$price");
SQL;
/*$sql = 'INSERT INTO bookstore.book (isbn,title,author,stock,price) VALUES (?,?,?,?,?)';
$result->bindValue(0,$email);*/
    ...
    public function insertBorrowedBooks($pdo, $customer_id, $book_id, $start) {
        $query = <<<SQL
INSERT INTO bookstore.borrowed_books (customer_id,book_id,start) VALUES ("$customer_id","$book_id","$start");
SQL;
        $result = $pdo->prepare($query);
        $result = $result->execute();
        if (!$result) {
    ...
    public function insertCustomer($pdo) {
        $query = <<<SQL
INSERT INTO bookstore.customer (firstname,surname,email,password,type) VALUES ("$this->firstname","$this->surname","$this->email","$this->password","$this->type");
SQL;
        $result = $pdo->prepare($query);
    ...
    public function insertSale($pdo, $customerID, $dateSale) {
        $query = <<<SQL
INSERT INTO bookstore.sale (customer_id,date) VALUES ("$customerID","$dateSale");
SQL;
        $result = $pdo->prepare($query);
        $result = $result->execute();
        if (!$result) {
    ...
    public function insertSaleBook($pdo, $bookID, $saleID, $amount) {
        $query = <<<SQL
INSERT INTO bookstore.sale_book (book_id,sale_id,amount) VALUES ("$bookID","$saleID","$amount");
SQL;
        $result = $pdo->prepare($query);
        $result = $result->execute();
        if (!$result) {
...
FROM bookstore.book, bookstore.borrowed_books
...
SQL;
    ...
    public function selectBookID($pdo, $title) {
        $query = <<<SQL
SELECT id from bookstore.book WHERE book.title=:title;
SQL;
        $result = $pdo->prepare($query);
    ...
    public function selectIDBookFromBorrowed($pdo, $title, $customerID) {
        $query = <<<SQL
SELECT id FROM bookstore.book, bookstore.borrowed_books
...
SQL;
    ...
    public function selectBookIDsFromBorrowed($pdo, $customerID) {
        $query = <<<SQL
SELECT book_id from bookstore.borrowed_books WHERE borrowed_books.customer_id=:customer_id AND end IS NULL;
SQL;
        $result = $pdo->prepare($query);
        $result->bindValue('customer_id', "$customerID");
        $result->execute();
    ...
    public function selectCustomerID($pdo, $email) {
        $query = <<<SQL
SELECT id from bookstore.customer WHERE customer.email=:email;
SQL;
        $result = $pdo->prepare($query);
    ...
    public function selectEmails($pdo) {
        $query = <<<SQL
SELECT email from bookstore.customer;
SQL;
        $result = $pdo->prepare($query);
        $result->execute();
        $rows = $result->fetchAll();
    ...
    public function selectPassword($pdo, $email) {
        $query = <<<SQL
SELECT password from bookstore.customer WHERE customer.email=:email;
SQL;
        $result = $pdo->prepare($query);
        $result->bindValue('email', "$email");
        $result->execute();
    ...
    public function selectSaleID($pdo, $customerID, $dateSale) {
        $query = <<<SQL
SELECT id from bookstore.sale WHERE sale.customer_id=:customer_id AND sale.date=:date;
SQL;
            $result = $pdo->prepare($query);
            $result->bindValue('customer_id', "$customerID");
    ...
    public function selectStock($pdo, $bookID) {
        $query = <<<SQL
SELECT stock FROM bookstore.book WHERE id=:id;
SQL;
        $result = $pdo->prepare($query);
        $result->bindValue('id', "$bookID");
        $result->execute();
    ...
    public function selectTitles($pdo) {
        $query = <<<SQL
SELECT title FROM bookstore.book;
SQL;
        $result = $pdo->prepare($query);
        $result->execute();
        $rows = $result->fetchAll();
    ...
    public function selectTitlesBorrowed($pdo, $bookID) {
        $query = <<<SQL
SELECT title FROM bookstore.book WHERE id=:book_id;
SQL;
        $result = $pdo->prepare($query);
    ...
    public function updateBorrowedBooks($pdo, $end, $customerID, $bookID) {
        $query = <<<SQL
UPDATE bookstore.borrowed_books SET end=:end WHERE borrowed_books.customer_id=:customer_id AND borrowed_books.book_id=:book_id;
SQL;
        $result = $pdo->prepare($query);
    ...
    public function updateStock($pdo, $newStock, $bookID) {
        $query = <<<SQL
UPDATE bookstore.book SET stock=:stock WHERE book.id=:book_id;
SQL;
        $result = $pdo->prepare($query);
